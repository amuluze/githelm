...
pub fn add_server(state: State<'_, AppState>, input: AddServerInput) -> AppResult<Server> {
    ...
    // Credential goes to the OS keychain (never SQLite, never the renderer).
    let entry = Entry::new(SERVICE, &server_credential_key(&server.id))
        .map_err(|e| AppError::Internal(format!("keyring: {e}")))?;
    entry
        .set_password(&input.credential)
